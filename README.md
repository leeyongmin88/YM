...
매월 예산은 **`Raw/예산.xlsx`** 파일에서 관리한다.
열: `구분 | 매체 | 통합매체 | 패턴 | MI | EBM | IT`
- **예산 변경**: `MI/EBM/IT` 값만 수정 → 실행하면 **Total [매체 예산 집행율] · 브랜드 종합 집행율 · 정액 집행액**에 모두 자동 반영 (단일 소스).
- **매체 추가**: 한 줄 추가하고 `구분`·`매체`·`통합매체`(통합 시트의 매체값, 예: Meta/Naver)·`패턴`(캠페인명 일부, 없으면 공백)·예산 기입 → 자동 반영.
  - ⚠️ 단, **데이터가 이미 들어오는 매체**만 잡힘. 완전히 새로운 플랫폼(새 RAW 소스)은 `ingest.py`에 리더 추가 필요.
- 파일이 없으면 코드 기본값(`total.py` MEDIA_ROWS) 사용.
- 분실 시 `pipeline/make_budget_template.py` 실행으로 템플릿 재생성.
